...
export const essayDrafts: Record<Exclude<Locale, 'en'>, EssayDraft> = {
  ru: {
    ...
    gapCaption: 'Поля далеко под диагональю показывают высокую техническую активность при ограниченных публичных полномочиях. Выберите точку, чтобы увидеть причины её положения.',
  ...
  ko: {
    ...
    gapCaption: '대각선 아래의 분야일수록 기술 활동에 비해 공적 권한과 책임성이 제한적입니다. 점을 선택하면 그 위치의 배경을 볼 수 있습니다.',
  ...
  fr: {
    ...
    gapCaption: 'Les domaines très en dessous de la diagonale combinent une forte activité technique et une autorité publique limitée. Sélectionnez un point pour comprendre sa position.',
  ...
  'zh-CN': {
    ...
    gapCaption: '远低于对角线的领域技术活动很强，但公共权力或问责有限。选择任意点可查看其位置背后的原因。',
